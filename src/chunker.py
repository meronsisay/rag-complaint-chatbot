...
class ComplaintChunker:
    def __init__(self, data_path='data/processed/filtered_complaints.csv', chunk_size=500, chunk_overlap=50):
        print("="*60)
        ...
        print(f" Configured text splitter: chunk_size={self.chunk_size}, overlap={self.chunk_overlap}")
        print("-"*60)
    ...
    def sample(self, size=12000, random_state=42):
        ...
        if len(self.sample_df) < size:
            ...
            extra = remaining.sample(min(needed, len(remaining)), random_state=random_state)
    ...
    def chunk(self):
        ...
        for idx, row in tqdm(self.sample_df.iterrows(), total=len(self.sample_df), desc="Chunking"):
        ...
        print(f"   Average chunks per complaint: {len(self.chunks)/len(self.sample_df):.1f}")
    ...
    def test_search(self, query="Why are customers unhappy with credit card fees?", k=5):
