...
class ComplaintDataProcessor:
    ...
    def load_and_filter_dataset(self, chunksize=CHUNK_SIZE):
        ...
        for chunk in reader:
            ...
            if chunk_count % 10 == 0:
                print(f"  Processed {total_processed:,} rows... (kept {total_kept:,} so far)")
        ...
        if filtered_chunks:
            ...
            print("\nCategory Breakdown:")
            for category, count in self.filtered_df['Product_Category'].value_counts().items():
    ...
    def clean_narratives(self, df=None, narrative_col=NARRATIVE_COL):
        ...
        if 'Product_Category' in self.cleaned_df.columns:
            ...
            for category, count in self.cleaned_df['Product_Category'].value_counts().items():
